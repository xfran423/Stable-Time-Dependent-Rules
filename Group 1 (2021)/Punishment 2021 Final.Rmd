...
punishment_aggression_list_2021 <- list()
...
for(i in datex) { 
  for (g in actex){
    ...
    punishment_aggression_list_2021[[paste0("list", dater[i], actorer[g])]] <- output}}
...
punishment_aggression_list_2021 <- lapply(punishment_aggression_list_2021, function(x) arrange(x, date, full_time))



#put events side by side
punishment_side_by_side_2021 <- lapply(punishment_aggression_list_2021, function(x) mutate(x, actor_2 = lead(actor), 
                                                                                 actor_2_rank = lead(actor_rank), 
                                                                                 subject_2 = lead(subject), 
                                                                                 subject_2_rank = lead(subject_rank), 
                                                                                 full_time_2 = lead(full_time), 
...
#pick out punishment rule while keeping all dataframes in a list
 punishment_side_by_side_tested_2021 <- map(punishment_side_by_side_2021, ~ mutate(.x, initiated = ifelse(actor == counter
                                                                                                & subject_2 == counter
 ...
 punishment_huge_2021 <- bind_rows(punishment_side_by_side_tested_2021, .id = "column_label")
 
 #sort by time and date
 punishment_huge_2021_sorted <- punishment_huge_2021 %>%
   arrange(punishment_huge_2021$date, punishment_huge_2021$full_time)
...
#Remove duplicate events, events that have been represented twice, and events that could not confidently be counted as the start of punishment
 punishment_huge_2021_sorted_no_dupr <- subset(punishment_huge_2021_sorted, initiated != "no") 
 
 #subset for desired columns
 punishment_no_dupr <- subset(punishment_huge_2021_sorted_no_dupr, select = c(date, full_time, actor, actor_rank, subject, subject_rank, 
 ...
 punishment_real_2021 <- length(which(punishment_no_dupr$rule_followed == "yes"))
 

 punishment_real_2021_frame <- data.frame(punishment_real_2021)
 
 
  #create df of only rule follows

 punishment_doner_all_yes <-filter(punishment_no_dupr, rule_followed == "yes")
 
```

#individual rule follows
```{r}

#test for individual use of rule. How many times did each bird use rule
 unique(punishment_doner_all_yes$actor_2)
 length(unique(punishment_doner_all_yes$actor_2)
 )
 
 summary.pun.individ_2021 <- punishment_doner_all_yes %>% group_by(actor_2) %>%
   summarize (
     count =n())
 
 summary.pun.individ_2021
 ...
 punishment_nofollow <- setdiff(unique(t2021_aggression_only_ready$actor), unique(punishment_doner_all_yes$actor_2)
 )
 punishment_nofollow
 #Calculate Richness score for rule following
 #DO FIRST BEFORE ADDING ZEROS AND DONT DO AGAIN WITHOUT RUNNING ABOVE CODE
 punishment_richness <- length(summary.pun.individ_2021$actor_2)/18
########################################################################################
...
########################################################################################
 ...
 summary.pun.individ_2021 <- rbind(summary.pun.individ_2021, punishment_nofollow_zeros)
 
 

individ_pun_plot_2021 <- ggplot(summary.pun.individ_2021, aes(x = actor_2, y = count )) + 
...
individ_pun_plot_2021

#Calculate eveness metric
pun_for_even <- subset(summary.pun.individ_2021, select = c(count))
...
#follows similar procedure as calculating the count of rule follows, with some differences in the rule detection loop and the structure
 ...
 punishment_side_by_side_tested_time_2021 <-  map(punishment_side_by_side_2021, ~ mutate(.x, initiated = ifelse(actor == counter
                                                                                                & subject_2 == counter
                                                                                                & actor_rank > subject_rank
                                                                                                , "yes","no"), 
                                      rule_followed = ifelse( subject_2 == counter
 ...
 punishment_huge_2021_time <- bind_rows(punishment_side_by_side_tested_time_2021, .id = "column_label")
 
 #sort by time and date
 punishment_huge_2021_sorted_time <- punishment_huge_2021_time %>%
   arrange(punishment_huge_2021_time$date, punishment_huge_2021_time$full_time)
 
#remove any duplicate or double counted events 
 punishment_huge_2021_sorted_no_dupr_time <- subset(punishment_huge_2021_sorted_time, initiated != "no") 
 
 
 #subset for columns of interest
 punishment_no_dupr_time <- subset(punishment_huge_2021_sorted_no_dupr_time, select = c(date, full_time, actor, actor_rank, subject, subject_rank, 
                                                                         counter, actor_2, actor_2_rank, subject_2, subject_2_rank, full_time_2, date2, 
 ...
 #subset for columns of interest
 pun_length_ready_2021 <- subset( pun_length, select = c(time_to_pun, actor, subject, actor_2, subject_2, date_period))
 
 #obtain value for number of times rule was followed
 pun <- length(pun_length_ready_2021$time_to_pun)
 
 
 
 #summarize the speed of rule follows. Here we are checking whether the speed of the rule in question tends to be clustered
 summary.pun <- pun_length_ready_2021 %>% group_by(time_to_pun) %>%
   summarize (
     count =n())
 ...
 ggplot(pun_length_ready_2021, aes(x=time_to_pun)) + 
...
#run ids should be 1 through however many times you want to run through the permutation. Here 1000 times
 ...
 masterframe_punishment.df <- data.frame(runIDs = character(),
                                          actor = character(),
                                          subject = character(),
                                          date = character(),
                                          full_time = integer(),
...
#set seed
 set.seed(826)
 #seed = 23 for no restriction
 #seed = 218 for 8 sec
 #seed = 198 for 85 sec
 #seed = 826 for 336 sec
 #forloop starts here'
...
#rename df
 punishment_aggression_list_2021_random <- list_split_by_bird

   
   
   #put events side by side
   punishment_side_by_side <- lapply(punishment_aggression_list_2021_random, function(x) mutate(x, actor_2 = lead(actor), 
                                                                                    actor_2_rank = lead(actor_rank), 
                                                                                    subject_2 = lead(subject), 
                                                                                    subject_2_rank = lead(subject_rank), 
                                                                                    full_time_2 = lead(full_time), 
...
#for example, if punishment is detected for a bird A whos specific dataframe I am not looking at (im viewing bird Bs df) , this may be a false positive, as that bird A's interactions are not all listed in the dataframe of another bird B. In other words, bird A may have done something else to another bird (C, D, F etc.) that is not listed, as it would be only listed in that specific birds dataframe, that being Bird A's own dataframe 
   ...
   #combine dataframes into one huge dataframe
   punishment_huge_2021 <- bind_rows(punishment_side_by_side_tested, .id = "column_label")
  ...
  #that coresponds with that run which will allow you to sort/put them in ascending order later
   ...
   masterframe_punishment.df  <- rbind(masterframe_punishment.df, punishment_complete)
 }
 
 
 #######################################################LOOP#END#######################################################
 #write.csv(masterframe_punishment.df, "C:/Users/Xmanf/Desktop/Ch1 TDR files/punishment2021countmaster_336sec.csv")
...
#masterframe_punishment.df <- read.csv("C:/Users/Xmanf/Desktop/Ch1 TDR files/punishment2021countmaster_336sec.csv", header=TRUE, stringsAsFactors=FALSE)


 #put run #s in an ascending vector
 #create vector for number of time the rules were followed to be stored
 
 cum_run_IDS <- unique(masterframe_punishment.df$runIDs)
 ...
 for (j in cum_run_IDS) {
   len_of_punishment <- length(which(masterframe_punishment.df$rule_followed == "yes" & 
                                       masterframe_punishment.df$runIDs == j))
   
   random_times_punishment_followed <- c(random_times_punishment_followed, len_of_punishment)
 }
 
 
 rand_pun_.frame <- data.frame(random_times_punishment_followed)
 
 #visualize observed rule follow vs reference distribution
 pun_2021_count_plot <- ggplot(rand_pun_.frame, aes(x= random_times_punishment_followed)) + 
   geom_histogram(aes(y=..density..),      
                  binwidth=6,
                  colour="black", fill="grey") +
   geom_density() + 
   geom_vline(xintercept = punishment_real_2021, colour = "red", size = 2) + geom_density(lwd = 1, linetype = 1) + theme_minimal() + theme(axis.text = element_text(size=25), axis.title= element_blank()) +
  scale_x_continuous(name = "Count of Punishment events", breaks=seq(0, 210, 50)) +  
     coord_cartesian(xlim = c(0, 210), ylim = c(0,.05)) +
...
punishment_tally2021 <-length(which(rand_pun_.frame$random_times_punishment_followed > punishment_real_2021))

finalpunishment_tally2021 <- 1 - punishment_tally2021/length(rand_pun_.frame$random_times_punishment_followed)


#p = 0 for no restrictions less often 
#p = .457 for 8 sec ass often 
# p = 0 for 85 seconds less often 
#p = 0 for 336 less often 
...
head(punishment_aggression_list_2021[[1]])
...
punishment_huge_2021_time <- distinct(punishment_huge_2021_time, date, full_time, actor, actor_rank, subject, subject_rank, obs_period, date_period, actor_2, subject_2, full_time_2, date2, time_to_pun, .keep_all= TRUE) 
...
punishment_huge_2021_time <- filter(punishment_huge_2021_time, time_to_pun != 0)


date_x <- punishment_huge_2021_time$date
obs <- punishment_huge_2021_time$date_period
#separate data into a list with each df=a date so we can randomize within days
#sort_by_date <- function(x, i, y) {y <- filter(x, date == i )
#mutate(y, date_counter = i)}
...
for(i in datex) { 
  output <- filter(punishment_huge_2021_time, punishment_huge_2021_time$date_period == dater[i] )
   if (nrow(output) < 1) {output [ nrow(output) + 1 , ] <- NA
    } 
...
#create an empty masterframe to be filled with each run of the permutation
masterframe_punishment.df_time_2021 <- data.frame(runIDs = character(),
                             actor = character(),
                             subject = character(),
                             date = character(),
                             full_time = integer(),
...
#combine dataframes into one huge dataframe
loop.datalist_time_randomized_binded_tested_pun_2021 <- bind_rows(loop.data.randomized.time_to_rule.added, .id = "column_label")


 

#sort by time and date
loop.datalist_time_randomized_binded_tested_pun_2021 <- loop.datalist_time_randomized_binded_tested_pun_2021 %>%
  arrange(loop.datalist_time_randomized_binded_tested_pun_2021$date, loop.datalist_time_randomized_binded_tested_pun_2021$full_time)
...
#Remove duplicate events, events that have been represented twice, and events that could not confidently be counted as the start of punishment
testhugesorted_no_dup_rand_time <- subset(loop.datalist_time_randomized_binded_tested_pun_2021, initiated != "no") 
...
punishment_no_dup_rand_time <- subset(testhugesorted_no_dup_rand_time, select = c(date, full_time, actor, subject, 
 ...
 runIDs <- rep(loop.run, length(punishment_no_dup_rand_time$actor))
  ...
  punishment_complete_rand_time_2021 <- cbind(runIDs, punishment_no_dup_rand_time)
  
  
#put each runs data into teh masterframe
  masterframe_punishment.df_time_2021 <- rbind(masterframe_punishment.df_time_2021, punishment_complete_rand_time_2021)
  
}
#write.csv(masterframe_punishment.df_time_2021, "C:/Users/Xmanf/Desktop/Ch1 TDR files/pun2021timemaster.csv")

```


#compare rule speed to random
```{r}
#masterframe_punishment.df_time_2021 <- read.csv("C:/Users/Xmanf/Desktop/Ch1 TDR files/pun2021timemaster.csv", header=TRUE, stringsAsFactors=FALSE)

head(masterframe_punishment.df_time_2021)

#subset masterframe so that only rule follows are included
masterframe_punishment.df_time_2021_only_follows <- subset(masterframe_punishment.df_time_2021, rule_followed == "yes" )

#isolate time it takes to carry out rule...
masterframe_pun_time_sub.df_2021 <- subset( masterframe_punishment.df_time_2021_only_follows, select = c(actor, subject, actor_2,subject_2, time_to_pun,date_period))
head(masterframe_pun_time_sub.df_2021)



#add data label for the randomized rule speed
masterframe_pun_time_ready.df <- mutate(masterframe_pun_time_sub.df_2021, data_type = "randomized")
...
pun_length_ready_2021_label <- mutate(pun_length_ready_2021, data_type = "observed")


#combine df for observed rule speed and randomized rule speed into one df
pun_length_ready_2021_comb <- rbind(masterframe_pun_time_ready.df, pun_length_ready_2021_label )
unique(pun_length_ready_2021_comb$data_type)




#provide label for rule in question
pun_length_ready_2021_comb <- mutate(pun_length_ready_2021_comb, rule = "punortunism")


#make sure that rule speed is numeric
pun_length_ready_2021_comb$time_to_pun <- as.numeric(pun_length_ready_2021_comb$time_to_pun)


#observed vs randomized raw plot. quick look at how the distriubtuions compare
ggplot(pun_length_ready_2021_comb, aes(x=time_to_pun, fill = data_type)) + 
  geom_histogram(aes(y=..density..),      
                 binwidth=1) +#,
                 #colour="black", fill="white") + 
...
#pun_2021_mode_obs <- getmode(pun_length_ready_2021_label$time_to_pun)

#pun_2021_mode_rand <-getmode(masterframe_pun_time_ready.df$time_to_pun)

#bind mode to dfs
#pun_2021_mode <-  rbind(pun_2021_mode_obs,pun_2021_mode_rand)

#test for homogeneity of variance
leveneTest(time_to_pun ~ data_type, data = pun_length_ready_2021_comb)


#split violin plot of observed vs randomized rule speed
pun_violin_plot <- ggplot(pun_length_ready_2021_comb, aes(rule, time_to_pun, fill = data_type))  +
...
pun_2021_time_plot <- pun_violin_plot

pun_2021_time_plot

#calculate the means for observed and randomized rule speed
mean_obs_rand_punortunism_2021 <- pun_length_ready_2021_comb %>% group_by(data_type) %>%
  dplyr::summarize (mean(time_to_pun))

mean_obs_rand_punortunism_2021
...
m1_pun_2021 <- glmer(time_to_pun ~ data_type  + (1 | date_period) , data = pun_length_ready_2021_comb ,   control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun=2e5)), family = Gamma(link = "log"))
summary(m1_pun_2021)
...
confint_pun_2021 <- round(confint(m1_pun_2021, method = "Wald", level = 0.95), 3)
confint_pun_2021
#Tukey
summary(glht(m1_pun_2021, linfct = mcp(data_type="Tukey")), test = adjusted("none"))
#R2 Values
tab_model(m1_pun_2021)
...
pun_length_ready_2021$time_to_pun <- as.numeric(pun_length_ready_2021$time_to_pun)
...
summary.pun_length_ready_2021 <- pun_length_ready_2021 %>% group_by(time_to_pun) %>%
  dplyr::summarize (
    count =n())

summary.pun_length_ready_2021
#Add density column
summary.pun_length_ready_2021_density <- mutate(summary.pun_length_ready_2021, density = count/sum(count))
#remove count column
summary.pun_length_ready_2021_density_no_count <- subset(summary.pun_length_ready_2021_density, select = -c(count))
#reorder the columns
summary.pun_length_ready_2021_density_no_count_reordered <- summary.pun_length_ready_2021_density_no_count[, c(2, 1)]
head(summary.pun_length_ready_2021_density_no_count_reordered)
#convert to matrix
summary.pun_length_ready_2021_density_no_count_reordered_mat <- as.matrix(summary.pun_length_ready_2021_density_no_count_reordered)





masterframe_length_ready <- subset(masterframe_pun_time_sub.df_2021, select = c(time_to_pun))
...
pun_obs_vs_rand_emd <- emdist::emd(summary.pun_length_ready_2021_density_no_count_reordered_mat, summary.masterframe_length_ready_density_no_count_reordered_mat)
pun_obs_vs_rand_emd
#52.61723
