...
pass_along_aggression_list_2022G1 <- list()
...
for(i in datex) { 
  for (g in actex){
    ...
    pass_along_aggression_list_2022G1[[paste0("list", dater[i], actorer[g])]] <- output}}




#ensure df arranged by date and time
pass_along_aggression_list_2022G1 <- lapply(pass_along_aggression_list_2022G1, function(x) arrange(x, date, full_time))
...
pass_along_side_by_side <- lapply(pass_along_aggression_list_2022G1, function(x) mutate(x, actor_2 = lead(actor), 
                                                                                 actor_2_rank = lead(actor_rank), 
                                                                                 subject_2 = lead(subject), 
                                                                                 subject_2_rank = lead(subject_rank), 
                                                                                 full_time_2 = lead(full_time), 
...
pass_along_real_2022G1 <- length(which(pass_along_no_dupr$rule_followed == "yes"))


pass_along_real_2022G1_frame <- data.frame(pass_along_real_2022G1)

#created df of only follows
...
summary.pass.individ_2022G1 <- pass_along_doner_all_yes %>% group_by(actor_2) %>%
  dplyr::summarize (
    count =n())
...
#show Liz
#unique(`2022G1_interactions_cleaned`$actor)
...
pass_along_nofollow <- setdiff(unique(t2022_aggression_only_readyG1$actor), unique(pass_along_doner_all_yes$actor_2)
)
pass_along_nofollow
#DO FIRST BEFORE ADDING ZEROS AND DONT DO AGAIN WITHOUT RUNNING ABOVE CODE
#Calculate Richness score for rule following
pass_along_richness <- length(summary.pass.individ_2022G1$actor_2)/18
...
summary.pass.individ_2022G1 <- rbind(summary.pass.individ_2022G1, pass_along_nofollow_zeros)

# Plot
individ_pass_plot_2022G1 <- ggplot(summary.pass.individ_2022G1, aes(x = actor_2, y = count )) + 
...
pass_for_even <- subset(summary.pass.individ_2022G1, select = c(count))
...
#put df side by side
pass_along_side_by_side_time <- lapply(pass_along_aggression_list_2022G1, function(x) mutate(x, actor_2 = lead(actor), 
                                                                                      actor_2_rank = lead(actor_rank), 
                                                                                      subject_2 = lead(subject), 
                                                                                      subject_2_rank = lead(subject_rank), 
                                                                                      full_time_2 = lead(full_time), 
...
#subset for columns of interest
pass_length_ready_2022G1 <- subset( pass_length, select = c(time_to_pass, actor, subject, actor_2,subject_2, date_period))

p <- length(pass_length_ready_2022G1$time_to_pass)

#summarize the speed of rule follows. Here we are checking whether the speed of the rule in question tends to be clustered
summary.pass <- pass_length_ready_2022G1 %>% group_by(time_to_pass) %>%
  dplyr::summarize (
    count =n())
...
ggplot(pass_length_ready_2022G1, aes(x=time_to_pass)) + 
...
time_between_events2022G1 <- subset(pass_along_huge_sorted_time, select = c(time_to_pass))
time_between_events2022G1$time_to_pass <- as.numeric(time_between_events2022G1$time_to_pass)

mean_time_between_events2022G1 <- mean(time_between_events2022G1$time_to_pass, na.rm=TRUE)
...
masterframe_passalong_2022G1.df <- data.frame(runIDs = character(),
                                       actor = character(),
                                       subject = character(),
                                       date = character(),
                                       full_time = integer(),
...
#set seed
set.seed(799)
#seed for unrestricted = 42
#seed for 8 sec = 229
#seed for 85 sec = 804
#seed for 336 sec = 799
#forloop starts here'
...
list_split_by_bird <- lapply(list_split_by_bird, function(x) arrange(x, date, full_time))
 ...
 pass_along_aggression_list <- list_split_by_bird
  ...
  masterframe_passalong_2022G1.df  <- rbind(masterframe_passalong_2022G1.df, passalong_complete)
}

#######################################################LOOP#END#######################################################
#write.csv(masterframe_passalong_2022G1.df, "C:/Users/Xmanf/Desktop/Ch1 TDR files/passalong2022G1countmaster_336sec.csv")
...
#masterframe_passalong_2022G1.df <- read.csv("C:/Users/Xmanf/Desktop/Ch1 TDR files/passalong2022G1countmaster_336sec.csv", header=TRUE, stringsAsFactors=FALSE)


#put run #s in an ascending vector
#create vector for number of time the rules were followed to be stored
cum_run_IDS <- unique(masterframe_passalong_2022G1.df$runIDs)
...
for (j in cum_run_IDS) {
  len_of_passalongs <- length(which(masterframe_passalong_2022G1.df$rule_followed == "yes" & 
                                      masterframe_passalong_2022G1.df$runIDs == j))
  
  random_times_pass_along_followed <- c(random_times_pass_along_followed, len_of_passalongs)
}


rand_pass_.frame_2022G1 <- data.frame(random_times_pass_along_followed)


 pass_2022G1_count_plot <- ggplot(rand_pass_.frame_2022G1, aes(x=random_times_pass_along_followed)) + 
  geom_histogram(aes(y=..density..),      
                 binwidth=6,
                 colour="black", fill="grey") +
  geom_density() + 
  geom_vline(xintercept = pass_along_real_2022G1, colour = "red", size = 2) + geom_density(lwd = 1, linetype = 1) + theme_minimal() + theme(axis.text = element_text(size=25), axis.title= element_blank()) +
...
pass_tally2022G1 <-length(which(rand_pass_.frame_2022G1$random_times_pass_along_followed > pass_along_real_2022G1))

finalpass_tally2022G1 <- 1 - pass_tally2022G1/length(rand_pass_.frame_2022G1$random_times_pass_along_followed)
...
#write.csv(masterframe_passalong.df_time_2022G1, "C:/Users/Xmanf/Desktop/Ch1 TDR files/passalong2022G1timemaster.csv")
...
#masterframe_passalong.df_time_2022G1 <- read.csv("C:/Users/Xmanf/Desktop/Ch1 TDR files/passalong2022G1timemaster.csv")
head(masterframe_passalong.df_time_2022G1)

#subset masterframe so that only rule follows are included
masterframe_passalong.df_time_2022G1_only_follows <- subset(masterframe_passalong.df_time_2022G1, rule_followed == "yes" )

#isolate time it takes to carry out rule...
masterframe_passalong_time_sub.df_2022G1 <- subset( masterframe_passalong.df_time_2022G1_only_follows, select = c(actor, subject, actor_2,subject_2, time_to_pass,date_period))
head(masterframe_passalong_time_sub.df_2022G1)



#add data label for the randomized rule speed
masterframe_passalong_time_ready.df <- mutate(masterframe_passalong_time_sub.df_2022G1, data_type = "randomized")
...
pass_length_ready_2022G1_label <- mutate(pass_length_ready_2022G1, data_type = "observed")


#combine df for observed rule speed and randomized rule speed into one df
pass_length_ready_2022G1_comb <- rbind(masterframe_passalong_time_ready.df, pass_length_ready_2022G1_label )
unique(pass_length_ready_2022G1_comb$data_type)




#provide label for rule in question
pass_length_ready_2022G1_comb <- mutate(pass_length_ready_2022G1_comb, rule = "passalong")


#make sure that rule speed is numeric
pass_length_ready_2022G1_comb$time_to_pass <- as.numeric(pass_length_ready_2022G1_comb$time_to_pass)


#observed vs randomized raw plot. quick look at how the distriubtuions compare
ggplot(pass_length_ready_2022G1_comb, aes(x=time_to_pass, fill = data_type)) + 
  geom_histogram(aes(y=..density..),      
                 binwidth=1) +#,
                 #colour="black", fill="white") + 
...
#pass_2022G1_mode_rand <-getmode(masterframe_passalong_time_ready.df$time_to_pass)

#bind mode to dfs
#pass_2022G1_mode <-  rbind(pass_2022G1_mode_obs,pass_2022G1_mode_rand)

#test for homogeneity of variance
leveneTest(time_to_pass ~ data_type, data = pass_length_ready_2022G1_comb)


#split violin plot of observed vs randomized rule speed
pass_violin_plot <- ggplot(pass_length_ready_2022G1_comb, aes(rule, time_to_pass, fill = data_type))  +
...
pass_2022G1_time_plot <- pass_violin_plot

pass_2022G1_time_plot

#calculate the means for observed and randomized rule speed
mean_obs_rand_passalong_2022G1 <- pass_length_ready_2022G1_comb %>% group_by(data_type) %>%
  dplyr::summarize (mean(time_to_pass))



mean_obs_rand_passalong_2022G1
#obs 264.6401
#rand 232.7297
...
m1pass_2022G1 <- lme4::glmer(time_to_pass ~ data_type  + (1 | date_period) , data = pass_length_ready_2022G1_comb ,   control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun=2e5)), family = Gamma(link = "log"))
summary(m1pass_2022G1)
#p = 0.0839    
#FIX THIS IN MANUSCRIPT

#0.0839 
#find confidence intervals
confint_pass_2022G1 <- round(confint(m1pass_2022G1, method = "Wald", level = 0.95), 3)
confint_pass_2022G1
#tukey
summary(glht(m1pass_2022G1, linfct = mcp(data_type="Tukey")), test = adjusted("none"))
#find R2 value
tab_model(m1pass_2022G1)
...
pass_length_ready_2022G1$time_to_pass <- as.numeric(pass_length_ready_2022G1$time_to_pass)
...
summary.pass_length_ready_2022G1 <- pass_length_ready_2022G1 %>% group_by(time_to_pass) %>%
  dplyr::summarize (
    count =n())

summary.pass_length_ready_2022G1
#Add density column
summary.pass_length_ready_2022G1_density <- mutate(summary.pass_length_ready_2022G1, density = count/sum(count))
#remove count column
summary.pass_length_ready_2022G1_density_no_count <- subset(summary.pass_length_ready_2022G1_density, select = -c(count))
#reorder the columns
summary.pass_length_ready_2022G1_density_no_count_reordered <- summary.pass_length_ready_2022G1_density_no_count[, c(2, 1)]
head(summary.pass_length_ready_2022G1_density_no_count_reordered)
#convert to matrix
summary.pass_length_ready_2022G1_density_no_count_reordered_mat <- as.matrix(summary.pass_length_ready_2022G1_density_no_count_reordered)





masterframe_length_ready <- subset(masterframe_passalong_time_sub.df_2022G1, select = c(time_to_pass))
...
pass_obs_vs_rand_emd <- emdist::emd(summary.pass_length_ready_2022G1_density_no_count_reordered_mat, summary.masterframe_length_ready_density_no_count_reordered_mat)
