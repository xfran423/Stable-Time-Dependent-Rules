...
`2022_birdIDsg1` <- read_csv("2022_birdIDs.csv")


`2022_interactions_rawg1` <- read_csv("2022_interactions_group1.csv")
...
't2022_aggression_onlyG1' <- filter(behaveXday.directed2022G1, behavior %in% c("crowd","displace"))



#subset for time period of interest (Here we are subseting for the first stable period)

t2022_aggression_only_readyG1 <- t2022_aggression_onlyG1[t2022_aggression_onlyG1$date >= "2022-1-26" & t2022_aggression_onlyG1$date <= "2022-1-28", ]

test <- t2022_aggression_onlyG1 %>%
  arrange(t2022_aggression_onlyG1$date, t2022_aggression_onlyG1$full_time)
...
subject <- unique(t2022_aggression_only_readyG1$actor)
...
rank <- c(5,4,9,3,6,2,11,1,7,8,10)
...
#bind to df
unordered_subject_numbered <- merge(t2022_aggression_only_readyG1, subject_bnumber_bind2)
...
#bind to df
unordered_both_numbered2 <- merge(unordered_subject_numbered, actor_bnumber_bind2)
...
both_numbered_sorted2022G1 <- unordered_both_numbered2 %>%
...
practdrlist_ordered2022G1 <- lapply(practdrlist, function(x) arrange(x, date, full_time))
...
#test for individual use how many times did each individual bird use the rule in question
unique(retal_doner_all_yes$actor_2)
length(unique(retal_doner_all_yes$actor_2)
)

summary.retal.individ_2022G1 <- retal_doner_all_yes %>% group_by(actor_2) %>%
  dplyr::summarize (
    count =n())

summary.retal.individ_2022G1
...
retal_nofollow <- setdiff(unique(t2022_aggression_only_readyG1$actor), unique(retal_doner_all_yes$actor_2)
 )
#Calculate Richness score for rule following
#DO FIRST BEFORE ADDING ZEROS AND DONT DO AGAIN WITHOUT RUNNING ABOVE CODE
retal_richness <- length(summary.retal.individ_2022G1$actor_2)/length((unique(t2022_aggression_only_readyG1$actor)
))
...
summary.retal.individ_2022G1 <- rbind(summary.retal.individ_2022G1, retal_nofollow_zeros)




# Plot
individ_retal_plot_2022G1 <- ggplot(summary.retal.individ_2022G1, aes(x = actor_2, y = count )) + 
...
individ_retal_plot_2022G1

#Calculate eveness metric
retal_for_even <- subset(summary.retal.individ_2022G1, select = c(count))
...
#speed of rule follows
```{r}
#######################################################################################################
#follows similar procedure as calculating the count of rule follows, with some differences in the rule detection loop and the structur
...
#start with "both_numbered_sorted2022G1" as my dataset
...
#write.csv(masterframe_retaliation.df2022G1, "C:/Users/Xmanf/Desktop/Ch1 TDR files/retal2022G1countmaster_336sec.csv")
...
```{r}
#masterframe_retaliation.df2022G1 <- read.csv("C:/Users/Xmanf/Desktop/Ch1 TDR files/retal2022G1countmaster_336sec.csv", header=TRUE, stringsAsFactors=FALSE)
...
retal_tally2022G1 <-length(which(rand_retal_.frame2022G1$random_times_retaliation_followed > retal_real2022G1))
...
#p = 0.084 for no restirction as often 
# p = .186 for 8 sec as often
#p = .426 for 85 sec
#p = .16 for 336 As often
...
#write.csv(masterframe_retaliation.df_time_2022G1, "C:/Users/Xmanf/Desktop/Ch1 TDR files/retal2022G1timemaster.csv")
```
#compare rule speed to random

```{r}
#masterframe_retaliation.df_time_2022G1 <- read.csv("C:/Users/Xmanf/Desktop/Ch1 TDR files/retal2022G1timemaster.csv")
...
#ensure that time (seconds) is numeric
...
#observed vs randomized raw quick look at how the distributions compare
...
#leveneTest(time_to_retal ~ data_type, data = retal_length_ready_comb_2022G1)
...
#create function to subset for all instances in which a  particular bird "i" is an actor or subject of interaction
filtd <- function(x, i, y) {y <- filter(x, subject == i | actor == i )
mutate(y, counter = i)}
...
#Each dataframe is stored within a list to make the dataframes easy to work with
for(i in actor) { 
  output <- filtd(both_numbered_sorted2022G1, i) 
  practdrlist[[i]] <- output}

#ignore function for now
#created function to put consecutive events side by side
leadtdr <- function(x) mutate(x, actor_2 = lead(actor), subject_2 = lead(subject), time_2 = lead(time), date2 = lead(date))
...
```{r}
#successfully picked out retaliation rule while keeping all dataframes in a list (yay!!!)
#loop to set conditionals to pick out retaliation
...
retaliation_no_dupr2022G1 <- subset(testhugesorted_no_dupr, select = c(date, full_time, actor, subject, behavior, 
                                                               counter, actor_2, subject_2, full_time_2, date2, 
                                                               initiated, rule_followed))
...
retaliation_no_dupr_rule_time <- subset(testhugesorted_no_dupr_rule_time, select = c(date, full_time, actor, subject, behavior, 
                                                               counter, actor_2, subject_2, full_time_2, date2, 
                                                               initiated, rule_followed, time_to_retal))
...
ggplot(retal_length_ready2022G1, aes(x=time_to_retal)) + 
  geom_histogram(aes(y=..density..),      
                 ...
                 colour="black", fill="white") + 
  geom_density() +   scale_x_continuous(name = "length in s of retal events", limits = c(0,3000)) +
  ggtitle("2022 Group 1")



#ggplot(data = retal_length_ready, aes(x = time_to_retal)) +
  #geom_histogram()
...
#here is probably where I will put this into a list of dataframes separated by date

#want to keep date and time in order, only want the order of events randomized, so break off #date and time to be 
#binded back on later
full_time <- obs_period_pt_4$full_time
date_x <- obs_period_pt_4$date
obs <- obs_period_pt_4$date_period
#separate data into a list with each df=a date so we can randomize within days
#sort_by_date <- function(x, i, y) {y <- filter(x, date == i )
...
dater <- unique(obs)
datex <- c(1: length(unique(obs)))
#boop <- filter(both_numbered_sorted2022G1, both_numbered_sorted2022G1$date == datex [2])
...
#take out of list format and put into df format. bind the time and date back on so that they #have not been randomized
loop.data.randomized <- bind_rows(loop.datalist, .id = "column_label")
loop.data.randomized.time.added <- cbind(loop.data.randomized, full_time )
loop.data.randomized.time.added <- subset(loop.data.randomized.time.added, select = -c(xorderx))

 
  #loop.random <- sample(1:length(loop.data2$actor),length(loop.data2$actor))
  
  #loop_unordered.data_list <- cbind(loop.random, loop.data2)
  
 
  #loop_ordered.data <- loop_unordered.data[order(loop_unordered.data$loop.random),]
  
  #ready_loop.data <- cbind(loop_ordered.data, date, DT_seconds)
  
  
  # run code for rule
  #procedure for retaliation begins here
...
#Each dataframe is stored within a list to make the dataframes easy to work with
for(i in actor) { 
  output <- filtd(loop.data.randomized.time.added, i) 
  practdrlist[[i]] <- output}
...
#create empty vector for number of time the rules were followed to be stored
random_times_retaliation_followed2022G1 <- c()
...
random_times_retaliation_followed2022G1 <- c(random_times_retaliation_followed2022G1, len_of_retaliations)
}

#turn vector into a dataframe for analysis
rand_retal_.frame2022G1 <- data.frame(random_times_retaliation_followed2022G1)

#plot time (the x intercept is the # of time a rule was used in the real data, found with 
#"retal_real <- length(which(retaliation_no_dupr$rule_followed == "yes"))
#retal_real.frame <- data.frame(retal_real))"
ggplot(rand_retal_.frame2022G1, aes(x=random_times_retaliation_followed2022G1)) + 
  geom_histogram(aes(y=..density..),      
                 binwidth=1,
                 colour="black", fill="white") +
  geom_density() + 
  geom_vline(xintercept = retal_real2022G1, colour = "red", size = 2)
