...
#determine how many time sopportunism was followed
...
opp_doner_all_yes_2022G1 <-filter(opp_no_dupr, rule_followed == "yes")
...
length(unique(opp_doner_all_yes_2022G1$actor_2)
)

summary.opp.individ_2022G1 <- opp_doner_all_yes_2022G1 %>% group_by(actor_2) %>%
  summarize (
    count =n())
...
opp_nofollow <- setdiff(unique(t2022_aggression_only_readyG1$actor), unique(opp_doner_all_yes_2022G1$actor_2)
)
opp_nofollow

#Calculate Richness score for rule following
#DO FIRST BEFORE ADDING ZEROS AND DONT DO AGAIN WITHOUT RUNNING ABOVE CODE
opp_richness <- length(summary.opp.individ_2022G1$actor_2)/10
...
opp_for_even <- subset(summary.opp.individ_2022G1, select = c(count))
...
#alt pile  273.0309
 ...
 mean_opp_speed_2022G1.df <- mutate(opp_length_ready_2022G1, rule = "opp")
 
  mean_alt_pile_speed_2022G1.df <- mutate(opp_length_ready_alt_pile, rule = "alt_pile")

 mean_opp_alt_pile_comb_2022G1 <- rbind(mean_opp_speed_2022G1.df, mean_alt_pile_speed_2022G1.df)
 
 mean_opp_alt_pile_comb_2022G1$time_to_opp <- as.numeric(mean_opp_alt_pile_comb_2022G1$time_to_opp)
 
 mean_opp_alt_pile_comb_model_2022G1  <- glmer(time_to_opp ~ rule  + (1 | date_period)    , data = mean_opp_alt_pile_comb_2022G1  , control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun=2e5)), family = Gamma(link = "log"))
summary(mean_opp_alt_pile_comb_model_2022G1)
 #p = 0.0313 
```
...
masterframe_opp2022G1.df <- data.frame(runIDs = character(),
                                 actor = character(),
                                 subject = character(),
                                 date = character(),
                                 full_time = integer(),
...
#seed for unrestricted = 19
#seed for 8 sec = 105
#seed for 85 sec = 111
#seed for 336 sec = 473
#forloop starts here'
...
#for example, if opportunism is detected for a bird A whos specific dataframe I am not looking at (im viewing bird Bs df) , this may be a false positive, as that bird A's interactions are not all listed in the dataframe of another bird B. In other words, bird A may have done something else to another bird (C, D, F etc.) that is not listed, as it would be only listed in that specific birds dataframe, that being Bird A's own dataframe
  ...
  masterframe_opp2022G1.df  <- rbind(masterframe_opp2022G1.df, opp_complete)
}


#######################################################LOOP#END#######################################################
 #write.csv(masterframe_opp2022G1.df, "C:/Users/Xmanf/Desktop/Ch1 TDR files/opp2022G1countmaster_336sec.csv")
...
#masterframe_opp2022G1.df <- read.csv("C:/Users/Xmanf/Desktop/Ch1 TDR files/opp2022G1countmaster_336sec.csv", header=TRUE, stringsAsFactors=FALSE)
...
cum_run_IDS <- unique(masterframe_opp2022G1.df$runIDs)
...
for (j in cum_run_IDS) {
  len_of_opp <- length(which(masterframe_opp2022G1.df$rule_followed == "yes" & 
                               masterframe_opp2022G1.df$runIDs == j))
...
#visualize observed rule follow vs reference distribution
 opp_2022G1_count_plot <-ggplot(rand_opp_.frame_2022G1, aes(x=random_times_opp_followed)) + 
  geom_histogram(aes(y=..density..),      
                 binwidth=2,
...
opp_tally2022G1 <-length(which(rand_opp_.frame_2022G1$random_times_opp_followed > opp_real_2022G1))

finalopp_tally2022G1 <- 1 - opp_tally2022G1/length(rand_opp_.frame_2022G1$random_times_opp_followed)

#p = o for unrestricted
#p = 0.216 for 8 sec
#p = 0.043 for 85 sec
# p = 0 for 336
```
...
#randomize rule speed vs random
```{r}
...
#write.csv(masterframe_opp.df_time_2022G1, "C:/Users/Xmanf/Desktop/Ch1 TDR files/opp2022G1timemaster.csv")
...
#masterframe_opp.df_time_2022G1 <- read.csv("C:/Users/Xmanf/Desktop/Ch1 TDR files/opp2022G1timemaster.csv", header=TRUE, stringsAsFactors=FALSE)
...
ggplot(opp_length_ready_2022G1_comb, aes(x=time_to_opp, fill = data_type)) + 
  geom_histogram(aes(y=..density..),      
                 binwidth=1) +#,
                 #colour="black", fill="white") + 
...
leveneTest(time_to_opp ~ data_type, data = opp_length_ready_2022G1_comb)
