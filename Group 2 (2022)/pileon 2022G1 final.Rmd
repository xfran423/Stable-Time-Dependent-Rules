...
#find observed count of rule follows
```{r}
...
datelist__pile_2022G1 <- lapply(datelist__pile, function(x) mutate(x, actor_2 = lead(actor), subject_2 = lead(subject), 
...
pile_side_by_side_tested_2022G1 <- map(datelist__pile_2022G1, ~ mutate(.x, initiated = "yes", 
 ...
 pile_huge_2022G1 <- bind_rows(pile_side_by_side_tested_2022G1, .id = "column_label")
 
 #sort by time and date
 pile_huge_2022G1_sorted <- pile_huge_2022G1 %>%
   arrange(pile_huge_2022G1$date, pile_huge_2022G1$full_time)
...
#Remove duplicate events, events that have been represented twice, and events that could not confidently be counted as the start of pileon
 pile_huge_2022G1_sorted_no_dupr <- subset(pile_huge_2022G1_sorted, initiated != "no") 
 
 
#subset for desired columns
 pile_no_dupr <- subset(pile_huge_2022G1_sorted_no_dupr, select = c(date, full_time, actor, actor_rank, subject, subject_rank, 
...
#determine how many times rule was followed
 pile_real_2022G1 <- length(which(pile_no_dupr$rule_followed == "yes"))
 
 
 pile_real_2022G1_frame <- data.frame(pile_real_2022G1)
 
  #create df of only rule follows
 pile_doner_all_yes_2022G1 <-filter(pile_no_dupr, rule_followed == "yes")
...
summary.pile.individ_2022G1 <- pile_doner_all_yes_2022G1 %>% group_by(actor_2) %>%
  summarize (
    count =n())
...
unique(t2022_aggression_onlyG1$actor)
unique(t2022_aggression_onlyG1$subject)


pile_nofollow <- setdiff(unique(t2022_aggression_onlyG1$actor), unique(pile_doner_all_yes_2022G1$actor_2)
 )

#DO FIRST BEFORE ADDING ZEROS AND DONT DO AGAIN WITHOUT RUNNING ABOVE CODE
pile_richness <- length(summary.pile.individ_2022G1$actor_2)/length((unique(t2022_aggression_onlyG1$actor)
))
...
summary.pile.individ_2022G1 <- rbind(summary.pile.individ_2022G1, pile_nofollow_zeros)
...
# Plot
individ_pile_plot_2022G1 <- ggplot(summary.pile.individ_2022G1, aes(x = actor_2, y = count )) + 
...
pile_for_even <- subset(summary.pile.individ_2022G1, select = c(count))
...
#test for length of pile-ons in seconds
pile_side_by_side_tested_time_2022G1 <- map(datelist__pile_2022G1, ~ mutate(.x, initiated = "yes", 
                                      rule_followed = ifelse( subject == subject_2
                                                              & actor != actor_2
                                                              & full_time_2 - full_time > 0
                                                              & date2 == date,
 ...
 #combine dataframes into one huge dataframe
 pile_huge_time_2022G1 <- bind_rows(pile_side_by_side_tested_time_2022G1, .id = "column_label")
 
 #sort by time and date
 pile_huge_time_2022G1_sorted <- pile_huge_time_2022G1 %>%
   arrange(pile_huge_time_2022G1$date, pile_huge_time_2022G1$full_time)
 
#remove any duplicate or double counted events 
 pile_huge_time_2022G1_sorted_no_dupr <- subset(pile_huge_time_2022G1_sorted, initiated != "no") 
 
 #subset for columns of interest
 pile_huge_time_2022G1_sorted_no_dupr <- subset(pile_huge_time_2022G1_sorted_no_dupr, select = c(date, full_time, actor, actor_rank, subject, subject_rank, 
                                                                         actor_2, actor_2_rank, subject_2, subject_2_rank, full_time_2, date2, 
...
pile_length_2022G1 <- subset(pile_huge_time_2022G1_sorted_no_dupr, rule_followed != "no")


#subset for columns of interest
pile_length_ready_2022G1 <- subset( pile_length_2022G1, select = c(time_to_pile,actor, subject, actor_2, subject_2, date_period))

pi <- length(pile_length_ready_2022G1$time_to_pile)


#summarize the speed of rule follows. Here we are checking whether the speed of the rule in question tends to be clustered
summary.pile <- pile_length_ready_2022G1 %>% group_by(time_to_pile) %>%
  summarize (
    count =n())
...
#mean rule speed
mean(pile_length_ready_2022G1$time_to_pile)
# pile 33.44295 
 # opp-pile 38.5 


#quick glimpses of the distribution of rule speeds
ggplot(pile_length_ready_2022G1, aes(x=time_to_pile)) + 
...
```{r}
 ...
 mean(pile_length_ready_rank_down_2022G1$time_to_pile)
#pile     33.44295 
#opp pile   38.5 
 ...
 mean_pile_speed_rank_down_2022G1.df <- mutate(pile_length_ready_rank_down_2022G1, rule = "pile")
 
  mean_opp_pile_speed_2022G1.df <- mutate(pile_length_ready_opp_pile_2022G1, rule = "opp_pile")

 mean_pile_opp_pile_comb_2022G1 <- rbind(mean_pile_speed_rank_down_2022G1.df, mean_opp_pile_speed_2022G1.df)
 
 mean_pile_opp_pile_comb_2022G1$time_to_pile <- as.numeric(mean_pile_opp_pile_comb_2022G1$time_to_pile)
 
 mean_pile_opp_pile_comb_model_2022G1  <- glmer(time_to_pile ~ rule  + (1 | date_period)    , data = mean_pile_opp_pile_comb_2022G1  , control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun=2e5)), family = Gamma(link = "log"))
summary(mean_pile_opp_pile_comb_model_2022G1)
 #p =     0.862    

mean_pile_opp_pile_comb_model_2022G1 <- glm(time_to_pile ~ rule, data = mean_pile_opp_pile_comb_2022G1 , family = Gamma(link = "log"))
...
masterframe_pileon2022G1.df <- data.frame(runIDs = character(),
                                         actor = character(),
                                         subject = character(),
                                         date = character(),
                                         full_time = integer(),
...
#seed = 256 for no restriction
#seed =  919 for 8 sec
#seed = 705  for 85 seconds
#seed = 340 for 336 seconds
#set seed
set.seed(340)
#forloop starts here'
...
#put events sidebyside within date and within obs
datelist__pile_2022G1_rand <- lapply(shuffled_day_obs_list, function(x) mutate(x, actor_2 = lead(actor), subject_2 = lead(subject), 
                                                                         full_time_2 = lead(full_time), actor_2_rank = lead(actor_rank), subject_2_rank = lead(subject_rank), date2 = lead(date)))


#pick out pile on rule while keeping all dataframes in a list 
pile_side_by_side_tested_2022G1_rand <- map(datelist__pile_2022G1_rand, ~ mutate(.x, initiated = "yes", 
                                      rule_followed = ifelse( subject == subject_2
                                                              & actor != actor_2
                                                              & full_time_2 - full_time <= 336 
                                                              & full_time_2 - full_time > 0
   ...
   #combine dataframes into one huge dataframe
   pile_huge_2022G1 <- bind_rows(pile_side_by_side_tested_2022G1_rand, .id = "column_label")
  ...
  #subset for desired columns
  pile_on_almost_complete <- subset(pile_huge_2022G1_sorted_no_dupr, select = c(actor, subject, date, full_time, 
  ...
  masterframe_pileon2022G1.df <- rbind(masterframe_pileon2022G1.df, pile_on_complete)
}

  
#######################################################LOOP#END#######################################################
#write.csv(masterframe_pileon2022G1.df, "C:/Users/Xmanf/Desktop/Ch1 TDR files/pileon2022G1countmaster_336sec.csv")



```

#compare count of rule follows to random
```{r}

#masterframe_pileon2022G1.df <- read.csv("C:/Users/Xmanf/Desktop/Ch1 TDR files/pileon2022G1countmaster_336sec.csv", header=TRUE, stringsAsFactors=FALSE)

#put run #s in an ascending vector
#create vector for number of time the rules were followed to be stored
cum_run_IDS <- unique(masterframe_pileon2022G1.df$runIDs)
...
for (j in cum_run_IDS) {
  len_of_pileons <- length(which(masterframe_pileon2022G1.df$rule_followed == "yes" & 
                                      masterframe_pileon2022G1.df$runIDs == j))
  
  random_times_pile_on_followed <- c(random_times_pile_on_followed, len_of_pileons)
}


rand_pile_.frame_2022G1 <- data.frame(random_times_pile_on_followed)

#visualize observed rule follow vs reference distribution
pile_2022G1_count_plot <- ggplot(rand_pile_.frame_2022G1, aes(x=random_times_pile_on_followed)) + 
  geom_histogram(aes(y=..density..),      
                 binwidth=6,
                 colour="black", fill="grey") + 
  geom_vline(xintercept = pile_real_2022G1, colour = "red", size = 2)  +
...
pile_tally2022G1 <-length(which(rand_pile_.frame_2022G1$random_times_pile_on_followed < pile_real_2022G1))

finalpile_tally2022G1 <- 1- pile_tally2022G1/length(rand_pile_.frame_2022G1$random_times_pile_on_followed)

#p= .127 for not restriction
#p = .217 for 8 sec
#p = .049 for 85 sec
#p = .122 for 336 sec
```
...
#mark true pile ons 
pile_side_by_side_tested_time_2022G1_marked <- map(pile_side_by_side_tested_time_2022G1, ~ mutate(.x, t.o.f = ifelse(rule_followed == "yes", "true","false")))

pile_side_by_side_tested_time_2022G1_marked_sub <- c()

#remove mutated columns
for (i in datex){
  pile_side_by_side_tested_time_2022G1_marked_sub[[i]] <- subset(pile_side_by_side_tested_time_2022G1_marked[[i]], select = -c(actor_2, subject_2, actor_2_rank, subject_2_rank, full_time_2, date2, initiated, rule_followed ) )
}
...
pile_side_by_side_tested_time_2022G1_binded <- distinct(pile_side_by_side_tested_time_2022G1_binded, date, full_time, actor, actor_rank, subject, subject_rank, obs_period, date_period, actor_2, subject_2, full_time_2, date2, time_to_pile, .keep_all= TRUE) 
...
date_x <- pile_side_by_side_tested_time_2022G1_binded$date
obs <- pile_side_by_side_tested_time_2022G1_binded$date_period
...
for(i in datex) { 
  output <- filter(pile_side_by_side_tested_time_2022G1_binded, pile_side_by_side_tested_time_2022G1_binded$date_period == dater[i] )
   if (nrow(output) < 1) {output [ nrow(output) + 1 , ] <- NA
    } 
...
#write.csv(masterframe_pileon.df_time_2022G1, "C:/Users/Xmanf/Desktop/Ch1 TDR files/pile2022G1timemaster.csv")
```
...
#masterframe_pileon.df_time_2022G1 <- read.csv("C:/Users/Xmanf/Desktop/Ch1 TDR files/pile2022G1timemaster.csv")
#raw data 
head(masterframe_pileon.df_time_2022G1)

#subset masterframe so that only rule follows are included
masterframe_pileon.df_time_2022G1_only_follows <- subset(masterframe_pileon.df_time_2022G1, rule_followed == "yes" )

#isolate time it takes to carry out rule...
masterframe_pileon_time_sub.df_2022G1 <- subset( masterframe_pileon.df_time_2022G1_only_follows, select = c(actor, subject, actor_2,subject_2, time_to_pile,date_period))
head(masterframe_pileon_time_sub.df_2022G1)



#add data label for the randomized rule speed
masterframe_pileon_time_ready.df <- mutate(masterframe_pileon_time_sub.df_2022G1, data_type = "randomized")
...
pile_length_ready_2022G1_label <- mutate(pile_length_ready_2022G1, data_type = "observed")


#combine df for observed rule speed and randomized rule speed into one df
pile_length_ready_2022G1_comb <- rbind(masterframe_pileon_time_ready.df, pile_length_ready_2022G1_label )
unique(pile_length_ready_2022G1_comb$data_type)




#provide label for rule in question
pile_length_ready_2022G1_comb <- mutate(pile_length_ready_2022G1_comb, rule = "pileon")


#make sure that rule speed is numeric
pile_length_ready_2022G1_comb$time_to_pile <- as.numeric(pile_length_ready_2022G1_comb$time_to_pile)


#observed vs randomized raw plot. quick look at how the distriubtuions compare
ggplot(pile_length_ready_2022G1_comb, aes(x=time_to_pile, fill = data_type)) + 
  geom_histogram(aes(y=..density..),      
                 binwidth=1) +#,
                 #colour="black", fill="white") + 
...
#pile_2022G1_mode_rand <-getmode(masterframe_pileon_time_ready.df$time_to_pile)

#bind mode to dfs
#pile_2022G1_mode <-  rbind(pile_2022G1_mode_obs,pile_2022G1_mode_rand)

#test for homogeneity of variance
leveneTest(time_to_pile ~ data_type, data = pile_length_ready_2022G1_comb)


#split violin plot of observed vs randomized rule speed
pile_violin_plot <- ggplot(pile_length_ready_2022G1_comb, aes(rule, time_to_pile, fill = data_type))  +
...
pile_2022G1_time_plot <- pile_violin_plot

pile_2022G1_time_plot

#calculate the means for observed and randomized rule speed
mean_obs_rand_pileon_2022G1 <- pile_length_ready_2022G1_comb %>% group_by(data_type) %>%
  dplyr::summarize (mean(time_to_pile))

mean_obs_rand_pileon_2022G1
#obs 33.57516
#rand 226.27810


m1pile <- glmer(time_to_pile ~ data_type  + (1 | date_period)  , data = pile_length_ready_2022G1_comb ,   control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun=2e5)), family = Gamma(link = "log"))
summary(m1pile)
#p-value = 2e-16
...
confint_pile_2022G1 <- round(confint(m1pile2022G1, method = "Wald", level = 0.95), 3)
confint_pile_2022G1
#tukey
summary(glht(m1pile2022G1, linfct = mcp(data_type="Tukey")), test = adjusted("none"))
#find R2 value
tab_model(m1pile2022G1)
...
pile_length_ready_2022G1$time_to_pile <- as.numeric(pile_length_ready_2022G1$time_to_pile)
...
summary.pile_length_ready_2022G1 <- pile_length_ready_2022G1 %>% group_by(time_to_pile) %>%
  dplyr::summarize (
    count =n())

summary.pile_length_ready_2022G1
#Add density column
summary.pile_length_ready_2022G1_density <- mutate(summary.pile_length_ready_2022G1, density = count/sum(count))
#remove count column
summary.pile_length_ready_2022G1_density_no_count <- subset(summary.pile_length_ready_2022G1_density, select = -c(count))
#reorder the columns
summary.pile_length_ready_2022G1_density_no_count_reordered <- summary.pile_length_ready_2022G1_density_no_count[, c(2, 1)]
head(summary.pile_length_ready_2022G1_density_no_count_reordered)
#convert to matrix
summary.pile_length_ready_2022G1_density_no_count_reordered_mat <- as.matrix(summary.pile_length_ready_2022G1_density_no_count_reordered)





masterframe_length_ready <- subset(masterframe_pileon_time_sub.df_2022G1, select = c(time_to_pile))
...
pile_obs_vs_rand_emd <- emdist::emd(summary.pile_length_ready_2022G1_density_no_count_reordered_mat, summary.masterframe_length_ready_density_no_count_reordered_mat)
...
pile_mm_pt1 <- pile_length_ready_2022G1_comb
pile_mm_pt1$act_sub_comb <- paste0(pile_mm_pt1$actor, ",", pile_mm_pt1$subject, ",",  pile_mm_pt1$actor_2, ",", pile_mm_pt1$subject_2)

pile_length_ready_comb_2022G1_mm <- lmerMultiMember::weights_from_vector(pile_mm_pt1$act_sub_comb)



m1pile_2022G1_MM <- glmer(time_to_pile ~ data_type   + (1| date_period) + ( 1| individ_ID), data = pile_length_ready_2022G1_comb ,   control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun=2e5)), family = Gamma(link = "log"), memberships = list(individ_ID = pile_length_ready_comb_2022G1_mm))

summary(m1pile_2022G1_MM)
...
tab_model(m1pile2022G1)
report::report(m1pile2022G1)


summary(m1pile2022G1)
...
welch.rules_pile_vs_random <- oneway.test(time_to_pile ~ data_type, data = pile_length_ready_2022G1_comb, var.equal = FALSE)
welch.rules_pile_vs_random
# p-value = 2e-16
