...
opp_aggression_list_2022G2 <- list()
...
for(i in datex) { 
  for (g in actex){
    ...
    opp_aggression_list_2022G2[[paste0("list", dater[i], actorer[g])]] <- output}}
...
opp_aggression_list_2022G2 <- lapply(opp_aggression_list_2022G2, function(x) arrange(x, date, full_time))




#put events side by side
opp_side_by_side <- lapply(opp_aggression_list_2022G2, function(x) mutate(x, actor_2 = lead(actor), 
                                                                                 actor_2_rank = lead(actor_rank), 
                                                                                 subject_2 = lead(subject), 
                                                                                 subject_2_rank = lead(subject_rank), 
                                                                                 full_time_2 = lead(full_time), 
...
#Remove duplicate events, events that have been represented twice, and events that could not confidently be counted as the start of oppiation
opp_huge_sorted_no_dupr <- subset(opp_huge_sorted, initiated != "no") 


#subset fpr desired columns
...
#determine how many times opportunism was followed
opp_real_2022G2 <- length(which(opp_no_dupr$rule_followed == "yes"))



opp_real_2022G2_frame <- data.frame(opp_real_2022G2)


#22 forr opp

#create df of only follows
opp_doner_all_yes_2022G2 <-filter(opp_no_dupr, rule_followed == "yes")
...
unique(opp_doner_all_yes_2022G2$actor_2)
length(unique(opp_doner_all_yes_2022G2$actor_2)
)

summary.opp.individ_2022G2 <- opp_doner_all_yes_2022G2 %>% group_by(actor_2) %>%
  summarize (
    count =n())

summary.opp.individ_2022G2
...
opp_nofollow <- setdiff(unique(t2022_aggression_onlyG2$actor), unique(opp_doner_all_yes_2022G2$actor_2)
)
opp_nofollow

#Calculate Richness score for rule following
#DO FIRST BEFORE ADDING ZEROS AND DONT DO AGAIN WITHOUT RUNNING ABOVE CODE
opp_richness <- length(summary.opp.individ_2022G2$actor_2)/10
#did -1 because top rank cant ever be opportunistic since its highest rank
...
summary.opp.individ_2022G2 <- rbind(summary.opp.individ_2022G2, opp_nofollow_zeros)


individ_opp_plot_2022G2 <- ggplot(summary.opp.individ_2022G2, aes(x = actor_2, y = count )) + 
...
individ_opp_plot_2022G2

#Calculate eveness metric
opp_for_even <- subset(summary.opp.individ_2022G2, select = c(count))
...
#test for length of opportunism
...
#subset for columns of interest
opp_length_ready_2022G2 <- subset( opp_length, select = c(time_to_opp, actor, subject, actor_2,subject_2, date_period))

#obtain value for number of times rule was followed
opp <- length(opp_length_ready_2022G2$time_to_opp)


#summarize the speed of rule follows. Here we are checking whether the speed of the rule in question tends to be clustered
summary.opp <- opp_length_ready_2022G2 %>% group_by(time_to_opp) %>%
  summarize (
    count =n())
...
#calculate mean of opportunism
mean(opp_length_ready_2022G2$time_to_opp)
#opp 368.5357 
#alt pile 310.0263 

#quick glimpses of the distribution of rule speeds
ggplot(opp_length_ready_2022G2, aes(x=time_to_opp)) + 
...
#alt pile  310.0263
 ...
 mean_opp_speed_2022G2.df <- mutate(opp_length_ready_2022G2, rule = "opp")
 
  mean_alt_pile_speed_2022G2.df <- mutate(opp_length_ready_alt_pile, rule = "alt_pile")

 mean_opp_alt_pile_comb_2022G2 <- rbind(mean_opp_speed_2022G2.df, mean_alt_pile_speed_2022G2.df)
 
 mean_opp_alt_pile_comb_2022G2$time_to_opp <- as.numeric(mean_opp_alt_pile_comb_2022G2$time_to_opp)
 
 mean_opp_alt_pile_comb_model_2022G2  <- glmer(time_to_opp ~ rule  + (1 | date_period)    , data = mean_opp_alt_pile_comb_2022G2  , control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun=2e5)), family = Gamma(link = "log"))
summary(mean_opp_alt_pile_comb_model_2022G2)
 #p = 0.244     
```
...
masterframe_opp2022G2.df <- data.frame(runIDs = character(),
                                 actor = character(),
                                 subject = character(),
                                 date = character(),
                                 full_time = integer(),
...
set.seed(225) # set seed
#seed for unrestricted = 175
#seed for 8 sec = 141
#seed for 85 sec = 28
#seed for 336 = 225
...
list_split_by_bird <- lapply(list_split_by_bird, function(x) arrange(x, date, full_time))
  ...
  #rename df
 opp_aggression_list_2022G2 <- list_split_by_bird
...
#loop to set conditionals to pick out oppiation

#counter corresponds with which birds dataframe I am looking at and is used to control for false positives 
#for example, if oppiation is detected for a bird A whos specific dataframe I am not looking at (im viewing bird Bs df) , this may be a false positive, as that bird A's interactions are not all listed in the dataframe of another bird B. In other words, bird A may have done something else to another bird (C, D, F etc.) that is not listed, as it would be only listed in that specific birds dataframe, that being Bird A's own dataframe
  opp_side_by_side_tested <- map(opp_side_by_side, ~ mutate(.x, initiated = ifelse(subject == counter
                                                                                               & subject_2 == counter
                                                                                 & subject_2_rank < actor_2_rank
                                                                                 & actor != actor_2
...
#Remove duplicate events, events that have been represented twice, and events that could not confidently be counted as the start of oppiation
  ...
  masterframe_opp2022G2.df  <- rbind(masterframe_opp2022G2.df, opp_complete)
}



#write.csv(masterframe_opp2022G2.df, "C:/Users/Xmanf/Desktop/Ch1 TDR files/opp2022G2countmaster_336sec.csv")
...
#masterframe_opp2022G2.df <- read.csv("C:/Users/Xmanf/Desktop/Ch1 TDR files/opp2022G2countmaster_336sec.csv", header=TRUE, stringsAsFactors=FALSE)


#put run #s in an ascending vector
#create vector for number of time the rules were followed to be stored

cum_run_IDS <- unique(masterframe_opp2022G2.df$runIDs)
...
for (j in cum_run_IDS) {
  len_of_opp <- length(which(masterframe_opp2022G2.df$rule_followed == "yes" & 
                               masterframe_opp2022G2.df$runIDs == j))
...
#visualize observed rule follow vs reference distribution
 opp_2022G2_count_plot <-ggplot(rand_opp_.frame_2022G2, aes(x=random_times_opp_followed)) + 
  geom_histogram(aes(y=..density..),      
                 binwidth=2,
                 colour="black", fill="grey") +
  geom_density() + 
  geom_vline(xintercept = opp_real_2022G2, colour = "red", size = 2) + geom_density(lwd = 1, linetype = 1) + theme_minimal() + theme(axis.text = element_text(size=25), axis.title= element_blank())+
  scale_x_continuous(name = "Count of Opportunism events", breaks=seq(0, 50, 10)) +  
     coord_cartesian(xlim = c(0, 50), ylim = c(0,.125)) +
scale_y_continuous(breaks = seq(0, .125, .025)) 
 
opp_2022G2_count_plot
...
opp_tally2022G2 <-length(which(rand_opp_.frame_2022G2$random_times_opp_followed > opp_real_2022G2))

finalopp_tally2022G2 <- 1 - opp_tally2022G2/length(rand_opp_.frame_2022G2$random_times_opp_followed)

#p =  .29 for unrestricted 
#p = .327 for 8 sec
#p =  .26 for 85
# p = 0.492 for 336
```
...
#create an empty masterframe to be filled with each run of the permutation
masterframe_opp.df_time_2022G2 <- data.frame(runIDs = character(),
                             actor = character(),
                             subject = character(),
                             date = character(),
                             full_time = integer(),
...
opp_no_dup_rand_time <- subset(testhugesorted_no_dup_rand_time, select = c(date, full_time, actor, subject, 
 ...
 runIDs <- rep(loop.run, length(opp_no_dup_rand_time$actor))
  ...
  opp_complete_rand_time_2022G2 <- cbind(runIDs, opp_no_dup_rand_time)
  
  
#put each runs data into teh masterframe
  masterframe_opp.df_time_2022G2 <- rbind(masterframe_opp.df_time_2022G2, opp_complete_rand_time_2022G2)
  
}
#write.csv(masterframe_opp.df_time_2022G2, "C:/Users/Xmanf/Desktop/Ch1 TDR files/opp2022G2timemaster.csv")
...
#masterframe_opp.df_time_2022G2 <- read.csv("C:/Users/Xmanf/Desktop/Ch1 TDR files/opp2022G2timemaster.csv", header=TRUE, stringsAsFactors=FALSE)

head(masterframe_opp.df_time_2022G2)

#subset masterframe so that only rule follows are included
masterframe_opp.df_time_2022G2_only_follows <- subset(masterframe_opp.df_time_2022G2, rule_followed == "yes" )

#isolate time it takes to carry out rule...
masterframe_opp_time_sub.df_2022G2 <- subset( masterframe_opp.df_time_2022G2_only_follows, select = c(actor, subject, actor_2,subject_2, time_to_opp,date_period))
head(masterframe_opp_time_sub.df_2022G2)



#add data label for the randomized rule speed
masterframe_opp_time_ready.df <- mutate(masterframe_opp_time_sub.df_2022G2, data_type = "randomized")
...
opp_length_ready_2022G2_label <- mutate(opp_length_ready_2022G2, data_type = "observed")


#combine df for observed rule speed and randomized rule speed into one df
opp_length_ready_2022G2_comb <- rbind(masterframe_opp_time_ready.df, opp_length_ready_2022G2_label )
unique(opp_length_ready_2022G2_comb$data_type)




#provide label for rule in question
opp_length_ready_2022G2_comb <- mutate(opp_length_ready_2022G2_comb, rule = "opportunism")


#make sure that rule speed is numeric
opp_length_ready_2022G2_comb$time_to_opp <- as.numeric(opp_length_ready_2022G2_comb$time_to_opp)


#observed vs randomized raw plot. quick look at how the distriubtuions compare
ggplot(opp_length_ready_2022G2_comb, aes(x=time_to_opp, fill = data_type)) + 
  geom_histogram(aes(y=..density..),      
                 binwidth=1) +#,
                 #colour="black", fill="white") + 
...
#opp_2022G2_mode_obs <- getmode(opp_length_ready_2022G2_label$time_to_opp)

#opp_2022G2_mode_rand <-getmode(masterframe_opp_time_ready.df$time_to_opp)

#bind mode to dfs
#opp_2022G2_mode <-  rbind(opp_2022G2_mode_obs,opp_2022G2_mode_rand)

#test for homogeneity of variance
leveneTest(time_to_opp ~ data_type, data = opp_length_ready_2022G2_comb)


#split violin plot of observed vs randomized rule speed
opp_violin_plot <- ggplot(opp_length_ready_2022G2_comb, aes(rule, time_to_opp, fill = data_type))  +
...
opp_2022G2_time_plot <- opp_violin_plot

opp_2022G2_time_plot

#calculate the means for observed and randomized rule speed
mean_obs_rand_opportunism_2022G2 <- opp_length_ready_2022G2_comb %>% group_by(data_type) %>%
  dplyr::summarize (mean(time_to_opp))

mean_obs_rand_opportunism_2022G2
...
m1_opp_2022G2 <- glmer(time_to_opp ~ data_type  + (1 | date_period) , data = opp_length_ready_2022G2_comb ,   control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun=2e5)), family = Gamma(link = "log"))
summary(m1_opp_2022G2)

#p = 8.73e-07 
#UPDATE MANu
#manu updated. rememebr to update supplemental

#confidence intervals
confint_opp_2022G2 <- round(confint(m1_opp_2022G2, method = "Wald", level = 0.95), 3)
confint_opp_2022G2
#tukey
summary(glht(m1_opp_2022G2, linfct = mcp(data_type="Tukey")), test = adjusted("none"))
#R2 Values
tab_model(m1_opp_2022G2)
```
...
opp_length_ready_2022G2$time_to_opp <- as.numeric(opp_length_ready_2022G2$time_to_opp)
...
summary.opp_length_ready_2022G2 <- opp_length_ready_2022G2 %>% group_by(time_to_opp) %>%
  dplyr::summarize (
    count =n())

summary.opp_length_ready_2022G2
#Add density column
summary.opp_length_ready_2022G2_density <- mutate(summary.opp_length_ready_2022G2, density = count/sum(count))
#remove count column
summary.opp_length_ready_2022G2_density_no_count <- subset(summary.opp_length_ready_2022G2_density, select = -c(count))
#reorder the columns
summary.opp_length_ready_2022G2_density_no_count_reordered <- summary.opp_length_ready_2022G2_density_no_count[, c(2, 1)]
head(summary.opp_length_ready_2022G2_density_no_count_reordered)
#convert to matrix
summary.opp_length_ready_2022G2_density_no_count_reordered_mat <- as.matrix(summary.opp_length_ready_2022G2_density_no_count_reordered)





masterframe_length_ready <- subset(masterframe_opp_time_sub.df_2022G2, select = c(time_to_opp))
...
opp_obs_vs_rand_emd <- emdist::emd(summary.opp_length_ready_2022G2_density_no_count_reordered_mat, summary.masterframe_length_ready_density_no_count_reordered_mat)
opp_obs_vs_rand_emd
#61.26587
```
